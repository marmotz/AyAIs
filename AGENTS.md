...
- **Build for Release**: `npm run electron:build` (generates local build without publishing)
- **Build and Publish**: `npm run electron:build:publish` (builds and publishes to GitHub releases)
...
- **Publish to GitHub**: Use `npm run electron:build:publish` to build and upload artifacts to GitHub releases
...
- **Build Command**: Always use `electron:build:publish` when preparing production releases
